...
def natural_sort_key(s):
    """
    Key function for natural sorting (numerical order)
    """
...
def main():
    ...
    images = [fn for fn in files if os.path.splitext(fn)[1].lower() in ('.jpg', '.jpeg', '.png', '.PNG')]
    
    if not images:
        print('No images for making collage! Please select other directory with images!')
        exit(1)

    # Sort by number using natural sorting
    images.sort(key=natural_sort_key)
    
    # shuffle images if needed (solo si se especifica el argumento -s)
    if args.shuffle:
